...
fn incompatible_ports_do_not_compile() {
    ...
    let output_directory =
        std::env::temp_dir().join(format!("orbita-sdk-compile-fail-{}", std::process::id()));
    ...
    let output = Command::new("rustc")
        ...
        .arg("--extern")
        .arg(format!("orbita_sdk={}", sdk.display()))
        .arg(concat!(
...
fn newest_sdk_library(dependencies: &std::path::Path) -> PathBuf {
    let mut candidates = fs::read_dir(dependencies)
        ...
        .filter(|path| {
            path.file_name()
                ...
                .is_some_and(|name| name.starts_with("liborbita_sdk-") && name.ends_with(".rlib"))
        })
        .collect::<Vec<_>>();
    ...
    candidates
        .pop()
        .expect("compiled orbita-sdk library should be available")
}
