//! End-to-end tests for `orbita validate` output and exit codes.
...
"#,
    ...
    let output = orbita(["validate".as_ref(), workflow.path().as_os_str()]);
...
fn invalid_workflow_uses_exit_code_one_and_json_diagnostics() {
    ...
    let Ok(report): Result<Value, _> = serde_json::from_slice(&output.stdout) else {
        panic!("orbita did not emit a JSON report");
    };
...
fn unreadable_path_and_bad_invocation_use_exit_code_two() {
    let missing = std::env::temp_dir().join(format!(
        "orbita-cli-missing-{}-{}.yaml",
        std::process::id(),
        NEXT_FIXTURE.fetch_add(1, Ordering::Relaxed)
    ));
    let missing_output = orbita(["validate".as_ref(), missing.as_os_str()]);
    let usage_output = orbita(["unknown".as_ref()]);
...
fn orbita<const N: usize>(arguments: [&std::ffi::OsStr; N]) -> Output {
    let result = Command::new(env!("CARGO_BIN_EXE_orbita"))
        .args(arguments)
        .output();
    let Ok(output) = result else {
        panic!("orbita process did not start");
    };
    output
}
