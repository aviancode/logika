...
/// Derives `logika_sdk::Schema` from a Rust struct or enum.
...
fn expand_schema(input: DeriveInput) -> syn::Result<TokenStream2> {
    ...
    Ok(quote! {
        impl #impl_generics ::logika_sdk::Schema for #ident #type_generics #where_clause {
            const NAME: &'static str = #name;
            const VERSION: u32 = #version_value;

            fn definition() -> ::logika_sdk::_private::SchemaDefinition {
                #definition
            }
        }
    })
...
fn schema_generics(mut generics: Generics) -> Generics {
    for parameter in generics.type_params_mut() {
        parameter.bounds.push(parse_quote!(::logika_sdk::Schema));
    }
    generics
}
...
fn fields_definition(fields: &Fields) -> syn::Result<TokenStream2> {
    ...
    Ok(quote! {
        ::logika_sdk::_private::SchemaDefinition::Struct(
            ::logika_sdk::_private::StructSchema::new([#(#fields),*])
        )
    })
}
    ...
    fields
        ...
        .map(|(index, field)| {
            ...
            if let Some(inner) = option_inner(&field.ty) {
                ...
                Ok(quote! {
                    ::logika_sdk::_private::SchemaField::optional(#name, #definition)
                })
            } else {
                let definition = type_definition(&field.ty)?;
                Ok(quote! {
                    ::logika_sdk::_private::SchemaField::required(#name, #definition)
                })
            }
        })
        .collect()
...
fn enum_definition(data: &DataEnum) -> syn::Result<TokenStream2> {
        ...
        .all(|variant| matches!(variant.fields, Fields::Unit))
    {
        ...
        return Ok(quote! {
            ::logika_sdk::_private::SchemaDefinition::Enum(
                ::logika_sdk::_private::EnumSchema::new([#(#variants),*])
            )
        });
    }
    ...
    let variants = data
        ...
        .map(|variant| {
            ...
            Ok(quote! {
                ::logika_sdk::_private::TaggedVariant::new(#tag, #definition)
            })
        })
        .collect::<syn::Result<Vec<_>>>()?;

    Ok(quote! {
        ::logika_sdk::_private::SchemaDefinition::TaggedUnion(
            ::logika_sdk::_private::TaggedUnionSchema::new([#(#variants),*])
        )
    })
}
...
fn type_definition(value: &Type) -> syn::Result<TokenStream2> {
    match value {
        Type::Array(array) => {
            ...
            Ok(quote!(::logika_sdk::_private::SchemaDefinition::array(#item)))
        }
        Type::Tuple(tuple) => {
            let fields = tuple
                .elems
...
fn path_definition(path: &TypePath) -> syn::Result<TokenStream2> {
    ...
    let primitive = match ident.as_str() {
        "bool" => Some(quote!(::logika_sdk::_private::PrimitiveType::Bool)),
        "i8" | "i16" | "i32" | "i64" | "isize" => {
            Some(quote!(::logika_sdk::_private::PrimitiveType::I64))
        }
        "u8" | "u16" | "u32" | "u64" | "usize" => {
            Some(quote!(::logika_sdk::_private::PrimitiveType::U64))
        }
        "f32" | "f64" => Some(quote!(::logika_sdk::_private::PrimitiveType::F64)),
        "String" | "str" => Some(quote!(::logika_sdk::_private::PrimitiveType::String)),
        _ => None,
    };
    if let Some(primitive) = primitive {
        return Ok(quote!(::logika_sdk::_private::SchemaDefinition::primitive(#primitive)));
    }
    ...
    match ident.as_str() {
        ...
        "Option" => {
            let arguments = type_arguments_from_segment(segment, 1)?;
            let item = type_definition(arguments[0])?;
            Ok(quote!(::logika_sdk::_private::SchemaDefinition::optional(#item)))
        }
        "Box" => {
            let arguments = type_arguments_from_segment(segment, 1)?;
        ...
        "BTreeMap" | "HashMap" => {
            ...
            Ok(quote!(::logika_sdk::_private::SchemaDefinition::map(#value)))
        }
        _ => Ok(quote!(<#path as ::logika_sdk::Schema>::definition())),
    }
}
