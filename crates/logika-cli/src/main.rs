//! Command-line entry point for `logika`.
...
use logika_workflow::{DecodeError, ValidationError, decode_json, decode_yaml, validate_workflow};
use serde_json::{Value, json};

const EXIT_VALID: u8 = 0;
const EXIT_INVALID: u8 = 1;
const EXIT_USAGE_OR_IO: u8 = 2;
const HELP: &str = "Usage: logika validate <WORKFLOW> [--format <human|json>]\n\
\n\
...
fn run(arguments: impl IntoIterator<Item = OsString>) -> Outcome {
    match parse_arguments(arguments) {
        ...
        Ok(Action::Version) => {
            println!("logika {}", env!("CARGO_PKG_VERSION"));
            Outcome::success()
        }
...
fn type_label(type_ref: &logika_core::TypeRef) -> String {
    format!(
        "{}@{}#{}",
        type_ref.name(),
        type_ref.version(),
